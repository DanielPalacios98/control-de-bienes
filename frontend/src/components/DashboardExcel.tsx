...
const DashboardExcel: React.FC<DashboardExcelProps> = ({ user }) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await inventoryAPI.delete(equipmentToDelete);
      loadData();
      setIsDeleteModalOpen(false);
      setEquipmentToDelete(null);
    } catch (error) {
      console.error('Error eliminando equipo:', error);
      alert('Error al eliminar equipo. Verifique si tiene movimientos asociados.');
    }
  };
            ...
            <div className="bg-white rounded-xl max-w-sm w-full shadow-2xl p-6 text-center">
              ...
              <div className="flex justify-center gap-3">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 font-medium transition-colors text-sm shadow-sm"
                >
                  Sí, Eliminar
                </button>
              </div>
